...
<input type="hidden" name="year" value="{{ $review->year }}"><input type="hidden" name="revision" value="{{ $review->revision }}">
...
@elseif($q['numeric'])<input type="text" inputmode="decimal" name="answers[facts][{{ $q['kod'] }}]" value="{{ data_get($answers,'facts.'.$q['kod']) }}" placeholder="Liczba (kropka dziesiętna) lub: nie wiem">
...
@for($i=0;$i<4;$i++)<h3>Wniosek {{ $i+1 }}</h3>@foreach(['finding'=>'Wniosek','decision'=>'Decyzja projektowa','document'=>'Dokument powiązany'] as $key=>$label)<label class="field"><span>{{ $label }}</span>@if($client)<div class="readonly">{{ data_get($answers,'conclusions.'.$i.'.'.$key)?:'Do uzupełnienia przez konsultanta' }}</div>@else<textarea name="answers[conclusions][{{ $i }}][{{ $key }}]">{{ data_get($answers,'conclusions.'.$i.'.'.$key) }}</textarea>@endif</label>@endforeach@endfor
</section>
</fieldset>
