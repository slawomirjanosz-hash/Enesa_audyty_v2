...
<nav class="aw-tabs" @if($clientView && $isIso50001) hidden @endif>@foreach(collect(['overview'=>'Podgląd','schedule'=>'Harmonogram i zadania','finances'=>'Finanse','documents'=>'Dokumenty','surveys'=>'Audyt','passports'=>'Paszporty Energetyczne'])->when(!$canViewFinances,fn($tabs)=>$tabs->forget('finances'))->when($clientView && $isIso50001,fn($tabs)=>$tabs->put('iso50001','ISO 50001')) as $id=>$label)<button class="aw-tab {{$loop->first?'active':''}}" data-audit-tab="{{$id}}" onclick="showAuditTab('{{$id}}',this)">{{$label}}</button>@endforeach</nav>
