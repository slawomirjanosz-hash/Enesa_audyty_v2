...
<nav class="aw-tabs">@foreach(collect(['overview'=>'Podgląd','schedule'=>'Harmonogram i zadania','finances'=>'Finanse','documents'=>'Dokumenty','surveys'=>'Ankiety Audytowe','passports'=>'Paszporty Energetyczne'])->when(!$canViewFinances,fn($tabs)=>$tabs->forget('finances'))->when($isIso50001,fn($tabs)=>$tabs->put('iso50001','ISO 50001')) as $id=>$label)<button class="aw-tab {{$loop->first?'active':''}}" data-audit-tab="{{$id}}" onclick="showAuditTab('{{$id}}',this)">{{$label}}</button>@endforeach</nav>
