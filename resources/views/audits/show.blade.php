...
<nav class="aw-tabs" @if($clientView && $isIso50001) hidden @endif>@foreach(collect(['overview'=>'Podgląd','schedule'=>'Harmonogram i zadania','finances'=>'Finanse','documents'=>'Dokumenty','surveys'=>'Audyty','passports'=>'Paszporty Energetyczne'])->when(!$canViewFinances,fn($tabs)=>$tabs->forget('finances'))->when($isIso50001,fn($tabs)=>$tabs->put('iso50001','ISO 50001')) as $id=>$label)<button class="aw-tab {{$loop->first?'active':''}}" data-audit-tab="{{$id}}" onclick="showAuditTab('{{$id}}',this)">{{$label}}</button>@endforeach</nav>
...
@if($isIso50001)<section id="aw-iso50001" class="aw-pane">@include('audits.partials.iso50001',['chapters'=>$isoChapters])</section>@endif
...
function showAuditTab(id,button){document.querySelectorAll('.aw-pane').forEach(p=>p.classList.toggle('active',p.id==='aw-'+id));document.querySelectorAll('.aw-tab').forEach(t=>t.classList.remove('active'));button.classList.add('active');const params=new URLSearchParams(location.search);params.set('tab',id);if(id!=='iso50001')params.delete('section');history.replaceState(null,'','?'+params.toString());if(id==='schedule')requestAnimationFrame(()=>{initProjectGantt();focusLinkedAuditTask()})}
