...
@push('styles')
@include('audits.schedule-styles')
<style>
...
<nav class="aw-tabs" @if($clientView && $isIso50001) hidden @endif>@foreach(collect(['overview'=>'Podgląd','schedule'=>'Harmonogram i zadania','finances'=>'Finanse','documents'=>'Dokumenty','surveys'=>'Audyty','passports'=>'Paszporty Energetyczne'])->when(!$canViewFinances,fn($tabs)=>$tabs->forget('finances'))->when($clientView && $isIso50001,fn($tabs)=>$tabs->put('iso50001','ISO 50001')) as $id=>$label)<button class="aw-tab {{$loop->first?'active':''}}" data-audit-tab="{{$id}}" onclick="showAuditTab('{{$id}}',this)">{{$label}}</button>@endforeach</nav>
...
<section id="aw-surveys" class="aw-pane">@include('audits.partials.surveys')</section>
...
@if($isIso50001)<section id="aw-iso50001" class="aw-pane">@unless($clientView)<div class="aw-actions"><button type="button" class="aw-btn soft" onclick="showAuditTab('surveys')">← Wróć do listy audytów</button></div>@endunless @include('audits.partials.iso50001',['chapters'=>$isoChapters])</section>@endif
...
<script>
const projectTimelineItems=@json($timelineItems),projectCanEdit=@json($canManageSchedule),projectStartDate=@json($audit->start_date?->format('Y-m-d')),projectEndDate=@json($audit->end_date?->format('Y-m-d')),projectCsrfToken=document.querySelector('meta[name="csrf-token"]')?.content||@json(csrf_token()),ganttBulkDeleteUrl=@json(route('audits.tasks.bulk-destroy',$audit));let projectGantt=null,projectGanttMode='Week';
...
function focusLinkedAuditTask() {
    const taskId = new URLSearchParams(location.search).get('task');
    if (!taskId || !/^\d+$/.test(taskId)) return;
    const row = document.getElementById('audit-task-' + taskId);
    if (!row) {
        const container = document.getElementById('gantt-task-list');
        if (container && !document.getElementById('audit-task-missing')) {
            const notice = document.createElement('p');
            notice.id = 'audit-task-missing';
            notice.textContent = 'Zadanie z linku nie jest już dostępne w tym audycie. Mogło zostać usunięte.';
            container.prepend(notice);
        }
        return;
    }
    row.classList.add('linked-audit-task');
...
function showAuditTab(id,button){if(!document.getElementById('aw-'+id))return;document.querySelectorAll('.aw-pane').forEach(p=>p.classList.toggle('active',p.id==='aw-'+id));document.querySelectorAll('.aw-tab').forEach(t=>t.classList.remove('active'));(button||document.querySelector('.aw-tab[data-audit-tab="'+(id==='iso50001'?'surveys':id)+'"]'))?.classList.add('active');const params=new URLSearchParams(location.search);params.set('tab',id);if(id!=='iso50001')params.delete('section');history.replaceState(null,'','?'+params.toString());if(id==='schedule')requestAnimationFrame(()=>{initProjectGantt();focusLinkedAuditTask()})}
...
document.addEventListener('DOMContentLoaded',()=>{const tab=new URLSearchParams(location.search).get('tab')||@json($clientView && $isIso50001 ? 'iso50001' : null);if(tab){const button=document.querySelector('.aw-tab[data-audit-tab="'+tab+'"]');showAuditTab(tab,button)}document.getElementById('audit-finance-search')?.addEventListener('input',e=>{const q=e.target.value.toLocaleLowerCase('pl');document.querySelectorAll('#audit-finance-rows tr[data-search]').forEach(row=>row.hidden=!row.dataset.search.toLocaleLowerCase('pl').includes(q))})});
