...
<section class="audit-iso-panel {{$loop->first?'active':''}}" data-iso-panel="{{$chapter['id']}}"><small style="font-weight:900;color:var(--green)">{{$chapter['number']}}</small><h2>{{$chapter['title']}}</h2><p>{{$chapter['description']}}</p><div class="iso-audit-note">Ten obszar audytu będzie uzupełniany kolejnymi uzgodnionymi polami.</div></section>
