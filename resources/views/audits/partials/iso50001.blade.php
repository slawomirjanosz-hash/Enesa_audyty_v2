...
<section class="audit-iso-panel {{$loop->first?'active':''}}" data-iso-panel="{{$chapter['id']}}"><h2><span>{{$chapter['number']}}</span> {{$chapter['title']}}</h2><p>{{$chapter['description']}}</p>@if($chapter['source_url'] ?? false)<a href="{{$chapter['source_url']}}" target="_blank" rel="noopener" style="display:inline-flex;margin-top:12px;color:var(--green);font-size:11px;font-weight:800;text-decoration:none"><i class="ti ti-external-link"></i>&nbsp; {{$chapter['source_label']}}</a>@endif @if($chapter['id']==='training')@include('audit-types.partials.iso50001-training',['canManageTraining'=>false])@elseif($chapter['id']!=='intro')<div class="iso-audit-note">Ten obszar audytu będzie uzupełniany kolejnymi uzgodnionymi polami.</div>@endif</section>
@foreach($chapter['items'] ?? [] as $item)<section class="audit-iso-panel" data-iso-panel="{{$item['id']}}"><h2><span>{{$item['number']}}</span> {{$item['title']}}</h2><p>{{$item['description']}}</p><div class="iso-clause"><div><strong>{{$item['number']}}</strong></div><div><strong>{{$item['title']}}</strong></div><div><p>{{$item['description']}}</p></div></div><div class="iso-audit-note">Tutaj pojawią się pola robocze tej części ISO 50001.</div></section>@endforeach
@endforeach
</div></div>
<script>
