...
<style>.plant-correction{margin:10px 0;padding:12px 14px;border-left:4px solid #2877b5;background:#edf6ff;border-radius:6px;color:#17466c;font-size:14px}.plant-correction p{margin:6px 0;white-space:pre-wrap;overflow-wrap:anywhere}.plant-question:has(.plant-correction)>input,.plant-question:has(.plant-correction)>select,.plant-question:has(.plant-correction)>details{border:2px solid #2877b5;background:#f3f8ff}</style>
@if(isset($profile->client_changes['_as_of_date']))<div class="plant-correction"><strong>Klient zmienił datę „Stan danych na dzień”</strong><p>Przed: {{$profile->client_changes['_as_of_date']['before']}} → Po korekcie: {{$profile->client_changes['_as_of_date']['after']}}</p></div>@endif
...
@php($change=$profile->$changeField[$q['key']])
<div class="plant-correction" @if($changeField==='auditor_changes') data-auditor-change="{{$q['key']}}" @else data-client-change="{{$q['key']}}" @endif><strong>Zmienione przez {{$changeAuthor}} — {{$change['by']}}</strong><small> · {{\Carbon\Carbon::parse($change['at'])->format('d.m.Y H:i')}}</small>
<p><strong>Przed zmianą:</strong> {{$questionnaire->display($q,$change['before'])}}</p><p><strong>Po korekcie {{$changeAuthor}}:</strong> {{$questionnaire->display($q,$change['after'])}}</p>
@foreach(['detail'=>'Uzupełnienie','source'=>'Źródło'] as $field=>$label)@if(($change['before'][$field]??null)!==($change['after'][$field]??null))<p><strong>{{$label}}:</strong> {{$change['before'][$field]??'Nie podano'}} → {{$change['after'][$field]??'Nie podano'}}</p>@endif @endforeach
</div>
@endif
@endforeach
