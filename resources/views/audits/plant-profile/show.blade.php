...
<style>.plant-correction{margin:10px 0;padding:12px 14px;border-left:4px solid #2877b5;background:#edf6ff;border-radius:6px;color:#17466c;font-size:14px}.plant-correction p{margin:6px 0;white-space:pre-wrap;overflow-wrap:anywhere}</style>
...
@if(isset($profile->auditor_changes[$q['key']]))
@php($change=$profile->auditor_changes[$q['key']])
<div class="plant-correction" data-auditor-change="{{$q['key']}}"><strong>Zmienione przez audytora — {{$change['by']}}</strong><small> · {{\Carbon\Carbon::parse($change['at'])->format('d.m.Y H:i')}}</small>
<p><strong>Przed zmianą:</strong> {{$questionnaire->display($q,$change['before'])}}</p><p><strong>Po korekcie audytora:</strong> {{$questionnaire->display($q,$change['after'])}}</p>
