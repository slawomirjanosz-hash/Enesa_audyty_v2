...
<div class="plant-scroll"><table><thead><tr><th>Zakład</th><th>Stan na dzień</th><th>Wersja</th><th>Status</th><th>Wypełnienie</th><th data-sortable="false">Akcje</th></tr></thead><tbody>
@foreach($profiles as $profile)<tr><td>{{ $profile->answers['site.name']['value'] ?? $sites->firstWhere('id',$profile->site_id)?->name }}</td><td data-sort-value="{{ $profile->as_of_date->format('Y-m-d') }}">{{ $profile->as_of_date->format('d.m.Y') }}</td><td data-sort-value="{{ $profile->revision }}">{{ $profile->revision }}</td><td>{{ \App\Models\IsoPlantProfile::STATUSES[$profile->status] }}</td><td data-sort-value="{{ app(\App\Services\QuestionnaireCompletion::class)->plant($profile->definition,$profile->answers)['percent'] }}">@include('partials.questionnaire-progress', ['progress'=>app(\App\Services\QuestionnaireCompletion::class)->plant($profile->definition,$profile->answers)])</td><td><a class="plant-button" href="{{ route($prefix.'show',[$audit,$profile]) }}">Otwórz profil</a></td></tr>@endforeach
</tbody></table></div>
