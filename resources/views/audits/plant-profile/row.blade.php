...
@else<input type="{{ $errors->has('answers.'.$q['key'].'.value.'.$rowIndex.'.'.$key) ? 'text' : $field['type'] }}" name="{{ $name }}[value][{{ $rowIndex }}][{{ $key }}]" value="{{ $row[$key]??'' }}" @if($field['type']==='number') inputmode="decimal" min="0" step="any" @else maxlength="500" @endif>@endif</label>@endforeach
