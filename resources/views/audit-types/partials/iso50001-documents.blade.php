...
<style>.iso-docs-head p,.iso-doc-suggestions strong,.iso-doc-suggestions li,.iso-doc-title,.iso-doc-description,.iso-doc-empty{font-size:13px}.iso-doc-meta,.iso-doc-btn,.iso-doc-submit{font-size:12px}.iso-doc-box>summary{font-size:14px}.iso-doc-form input,.iso-doc-form textarea{font-size:13px}</style>
@endonce
...
<div class="iso-docs">
    ...
    <div class="iso-doc-columns">
        <details class="iso-doc-box" open><summary>Dokumentacja wzorcowa <span class="iso-doc-count">{{ $sectionTemplates->count() }}</span></summary><div class="iso-doc-body">
        ...
        @if($isAuditContext)
        <details class="iso-doc-box" open><summary>Dokumentacja klienta <span class="iso-doc-count">{{ $sectionClientDocuments->count() }}</span></summary><div class="iso-doc-body">
            @forelse($sectionClientDocuments as $document)
                <div class="iso-doc-row"><div class="iso-doc-title">{{ $document->title }}</div><div class="iso-doc-meta">Rok {{ $document->document_year ?? '—' }} · wersja {{ $document->version_number }} · {{ $document->formattedSize() }} · {{ $document->uploader?->name ?? 'System' }}</div>@if($document->description)<div class="iso-doc-description">{{ $document->description }}</div>@endif
                    <div class="iso-doc-actions"><a class="iso-doc-btn" href="{{ $clientView ? route('client.audits.iso-documents.download', [$audit, $document]) : route('audits.iso-documents.download', [$audit, $document]) }}"><i class="ti ti-download"></i> Pobierz</a>
                    @if($canUpload)<form method="POST" action="{{ route('audits.iso-documents.destroy', [$audit, $document]) }}">@csrf @method('DELETE')<button class="iso-doc-btn danger" onclick="return confirm('Usunąć tę wersję dokumentu?')"><i class="ti ti-trash"></i> Usuń</button></form>@endif</div>
                </div>
            @empty<div class="iso-doc-empty">Nie dodano jeszcze dokumentacji klienta.</div>@endforelse
            @if($canUpload)
