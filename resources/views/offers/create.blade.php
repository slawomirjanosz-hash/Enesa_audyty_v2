...
<div id="modal-template-pick" style="display:flex;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;align-items:center;justify-content:center;padding:20px;">
    <div style="background:#fff;border-radius:16px;padding:32px;width:100%;max-width:560px;box-shadow:0 20px 60px rgba(0,0,0,.25);">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:24px;">
            ...
            @if(!empty($pricingSuggestion['rows']) || !empty($pricingSuggestion['delegations']))
            <button onclick="pickTemplate(null, true)" style="display:flex;align-items:center;gap:16px;background:#FFFBEB;border:2px solid #FCD34D;border-radius:10px;padding:16px 20px;cursor:pointer;text-align:left;transition:border-color .15s;" onmouseover="this.style.borderColor='#D97706'" onmouseout="this.style.borderColor='#FCD34D'">
                ...
                <div>
                    ...
                    <div style="font-size:12px;color:#775B16;">{{ $pricingSuggestion['matched_rules'] }} {{ $pricingSuggestion['matched_rules'] === 1 ? 'element' : 'elementy' }} · usługi: {{ number_format($pricingSuggestion['net_total'], 2, ',', ' ') }} zł netto · dojazd zostanie wyliczony z adresu</div>
                </div>
            </button>
            @endif
...
function applyPricingSuggestion() {
    ...
    const delegationSuggestions = Array.isArray(PRICING_SUGGESTION.delegations) ? PRICING_SUGGESTION.delegations : [];
    if (!rows.length && !delegationSuggestions.length) return;
    ...
    if (delegationSuggestions.length && typeof delegRender === 'function') {
        delegSections = delegationSuggestions;
        delegRender();
        delegationSuggestions.forEach((_, index) => delegFetchKm(index));
    }
    recalcAll();
}
