@extends($layout)
@section('title', 'Przeglądy butli')
@section('page-title', 'Przeglądy butli')
@section('content')
...
<tr><td><strong>{{ $cylinder->serial_number }}</strong></td><td>{{ $cylinder->company?->name }}</td><td>{{ $cylinder->type }}<br><span class="cyl-muted">{{ $cylinder->manufacturer }}</span></td><td>{{ $cylinder->latestInspection?->inspected_at?->format('d.m.Y') ?? 'Brak wpisu' }}</td><td class="{{ $cylinder->latestInspection?->next_due_at?->lt(today()) ? 'cyl-overdue' : '' }}">{{ $cylinder->latestInspection?->next_due_at?->format('d.m.Y') ?? 'Nie ustalono' }}</td><td><a class="cyl-btn" href="{{ route($routePrefix.'show', $cylinder) }}">Otwórz</a></td></tr>
...
@unless($clientView)<div class="cyl-notice">Pierwszy etap modułu: rejestr i ręczne wpisy przeglądów. Analiza zdjęć AI, import pomiarów i protokoły będą dodawane po ustaleniu procedury badania.</div>@endunless
</div>
@endsection
