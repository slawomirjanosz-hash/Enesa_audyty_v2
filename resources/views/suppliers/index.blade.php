...
function fetchSupplierFromGus(){const nipInput=document.getElementById('supplier-nip');const nip=nipInput.value.replace(/[^0-9]/g,'');const status=document.getElementById('supplier-gus-status');if(nip.length!==10){status.style.color='#b91c1c';status.textContent='NIP musi mieć 10 cyfr.';return}status.style.color='#7b8580';status.textContent='Pobieranie danych z GUS...';fetch('{{route('companies.fetchGus')}}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content},body:JSON.stringify({nip})}).then(async response=>{const data=await response.json();if(!response.ok)throw new Error(data.error||'Nie udało się pobrać danych z GUS.');return data}).then(data=>{nipInput.value=nip;document.getElementById('supplier-company-name').value=data.name||'';document.getElementById('supplier-company-address').value=data.address||'';document.getElementById('supplier-company-city').value=data.city||'';status.style.color='#2e7d32';status.textContent='Dane pobrane poprawnie.'}).catch(error=>{status.style.color='#b91c1c';status.textContent=error.message})}
