...
<div id="auditCreateModal" class="user-modal-overlay" style="display:none" onclick="if(event.target===this)this.style.display='none'">
    ...
    <form method="POST" action="{{route('audits.store')}}">@csrf<input type="hidden" name="company_id" value="{{$company->id}}"><div class="modal-grid">
        <div class="modal-field" style="grid-column:1/-1"><label for="new-audit-type">Rodzaj audytu *</label><select id="new-audit-type" name="audit_type_id" required><option value="">Wybierz rodzaj audytu</option>@foreach($auditTypes as $auditType)<option value="{{$auditType->id}}" @selected(old('audit_type_id')==$auditType->id)>{{$auditType->name}}</option>@endforeach</select>@error('audit_type_id','auditCreate')<span style="color:#b42318">{{$message}}</span>@enderror @if($auditTypes->isEmpty())<small>Brak rodzajów audytu. Dodaj je w System audytów → Typy audytów.</small>@endif</div>
