...
<div class="project-grid">
    ...
    <a class="project-card" href="{{ route('projects.show',$project) }}">
        <div style="display:flex;justify-content:space-between;gap:10px;"><span class="project-number">{{ $project->number }}</span><span class="status">{{ ['planned'=>'Planowany','active'=>'Aktywny','on_hold'=>'Wstrzymany','completed'=>'Zakończony','cancelled'=>'Anulowany'][$project->status] ?? $project->status }}</span></div>
        <h2>{{ $project->name }}</h2>
        <div class="meta">
