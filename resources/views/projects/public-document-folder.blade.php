...
<html lang="pl">
<head>
    @php($brandColor = $companySettings?->primaryColor() ?: '#1A4D3A')
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="robots" content="noindex,nofollow">
    <title>{{ $share->folder->name }} — udostępnione dokumenty</title>
    <style>
        :root{--brand:{{ $brandColor }};--brand-soft:color-mix(in srgb,var(--brand) 7%,white);--brand-border:color-mix(in srgb,var(--brand) 42%,white)}*{box-sizing:border-box}body{margin:0;background:#f4f1ea;color:#1d2822;font-family:Arial,sans-serif}.top{background:var(--brand);color:#fff;padding:18px 24px}.top-inner,.wrap{width:min(980px,calc(100% - 28px));margin:auto}.brand{display:flex;align-items:center;gap:12px;font-weight:800}.brand img{max-height:36px;max-width:150px}.wrap{padding:32px 0}.card{background:#fff;border:1px solid #e1ddd3;border-radius:14px;padding:22px;margin-bottom:16px;box-shadow:0 8px 30px rgba(25,55,42,.05)}h1{font-size:23px;margin:0 0 7px}.lead{margin:0;color:#66736b}.notice{padding:12px 14px;border-radius:8px;background:var(--brand-soft);color:var(--brand);margin-bottom:16px;font-weight:700}.upload{display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:15px;background:var(--brand-soft);border:1px dashed var(--brand-border);border-radius:10px}.btn{border:0;border-radius:8px;padding:10px 15px;background:var(--brand);color:#fff;font-weight:800;cursor:pointer;text-decoration:none;display:inline-flex}.muted{color:#77827b;font-size:12px;margin-top:4px}.empty{text-align:center;color:#89928d;padding:28px}.table-wrap{overflow-x:auto}table{width:100%;border-collapse:collapse}th,td{padding:12px 9px;border-bottom:1px solid #eee;text-align:left;white-space:nowrap}th{font-size:12px;color:#657169}td:first-child{width:100%;white-space:normal}.file-name{font-weight:800;overflow-wrap:anywhere}.document-sort-button{display:inline-flex;align-items:center;gap:4px;padding:0;border:0;background:none;color:inherit;font:inherit;font-weight:800;cursor:pointer}.document-sort-button:after{content:'↕';color:#9aa39e}.document-sort-button[data-direction="asc"]:after{content:'↑';color:var(--brand)}.document-sort-button[data-direction="desc"]:after{content:'↓';color:var(--brand)}@media(max-width:650px){.card{padding:15px}th,td{padding:10px 7px}}
    </style>
</head>
<body>
