...
@endphp
<style>
.grid2{grid-template-columns:repeat(2,minmax(0,1fr))}.grid2>.field{min-width:0}.grid2>.field input,.grid2>.field select,.grid2>.field textarea{box-sizing:border-box;max-width:100%;width:100%}#gantt-task-modal .project-modal-box{width:min(820px,calc(100vw - 32px));overflow-x:hidden;overflow-y:auto}
    ...
    try {
        ...
        if (select.dataset.kind === 'finance') {
            ...
            if (row) {
                ...
                row.dataset.financeStatusSearch = data.status + ' ' + statusLabel;
                row.dataset.financeSortStatus = statusLabel;
            }
...
function initProjectGantt() {
    ...
    const tasks = projectTimelineItems.map(item => ({
        ...
        custom_class: item.is_milestone ? 'milestone-row' + (Number(item.progress) >= 100 ? ' done-milestone' : '') : 'task-row ' + ganttProgressClass(item.progress),
    }));
    if(projectEndDate){
...
const financeCollator = new Intl.Collator('pl', {numeric: true, sensitivity: 'base'});
document.querySelectorAll('[data-finance-sort]').forEach(button => button.addEventListener('click', () => {
    const key = button.dataset.financeSort;
    const direction = button.dataset.direction === 'asc' ? 'desc' : 'asc';
    const numeric = button.dataset.financeSortType === 'number';
    document.querySelectorAll('[data-finance-sort]').forEach(item => delete item.dataset.direction);
    button.dataset.direction = direction;
    financeRows.sort((left, right) => {
        const leftValue = left.dataset['financeSort' + key.charAt(0).toUpperCase() + key.slice(1)] || '';
        const rightValue = right.dataset['financeSort' + key.charAt(0).toUpperCase() + key.slice(1)] || '';
        const comparison = numeric
            ? Number(leftValue) - Number(rightValue)
            : financeCollator.compare(leftValue, rightValue);
        return direction === 'asc' ? comparison : -comparison;
    });
    const body = document.querySelector('#finance-register-table tbody');
    financeRows.forEach(row => body?.appendChild(row));
}));
