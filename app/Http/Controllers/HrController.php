...
class HrController extends Controller
{
    public function index(Request $request): View
    {
        ...
        return view('hr.index', compact('tab', 'users', 'trips', 'attendances', 'vehicles', 'canTeam', 'canDelegations', 'canAttendance', 'canAllVehicles', 'selectedUserId', 'defaultKmRate', 'defaultDietRate'));
    }
    ...
    public function tripPdf(Request $request, HrBusinessTrip $trip): Response
    {
        ...
        return Pdf::loadView('hr.trip-pdf', ['trip' => $trip])->setPaper('a4')
    ...
    private function tripData(Request $request, ?int $forcedUserId = null): array
    {
        $data = $request->validateWithBag('trip', [
            ...
            'toll_cost' => ['nullable', 'numeric', 'min:0', 'max:9999999999'], 'notes' => ['nullable', 'string', 'max:3000'], 'remember_vehicle' => ['nullable', 'boolean'],
        ]);
        $user = $request->user();
        ...
        $data['total_amount'] = round($data['mileage_amount'] + $data['diet_amount'] + $data['toll_cost'], 2);
